...
def asus_benign_config():
    ...
    cfg.TRAIN_JSON_FILE = os.path.join("Annotations", "ASUS_Nodule", "benign_merge", "annotations_train.json")
    cfg.VALID_JSON_FILE = os.path.join("Annotations", "ASUS_Nodule", "benign_merge", "annotations_valid.json")
...
def asus_malignant_config():
    ...
    cfg = common_config()
    cfg.USING_DATASET.append(dataset_name)
    cfg.TRAIN_JSON_FILE = os.path.join("Annotations", "ASUS_Nodule", "malignant_merge", "annotations_train.json")
    cfg.VALID_JSON_FILE = os.path.join("Annotations", "ASUS_Nodule", "malignant_merge", "annotations_valid.json")
...
def dataset_config(cfg, using_dataset):
    ...
    for dataset_name in using_dataset:
        ...
        data_path = rf"C:\Users\test\Desktop\Leon\Datasets\{dataset_name}-preprocess\raw"
        # Prepare the dataset
        register_coco_instances(f"{dataset_name}-train", {}, train_json, data_path)
        register_coco_instances(f"{dataset_name}-valid", {}, valid_json, data_path)
    return cfg
