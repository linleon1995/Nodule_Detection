...
class LUNA16_CropRange_Builder():
    ...
    def build_luna16_crop_dataset(crop_range, save_path, volume_generator=luna16_volume_generator.Build_DLP_luna16_volume_generator()):
        ...
        for vol_idx, (raw_volume, target_volume, volume_info) in enumerate(volume_generator):
            ...
            positive, negative = LUNA16_CropRange_Builder.get_luna16_crop_range(
                raw_volume[...,0], nodule_center_xyz, crop_range_array, volume_info['origin'], volume_info['spacing'], volume_info['direction'])
            ...
            if positive is not None:
                ...
                positive_data = np.concatenate([pid_array, positive, category_array], axis=1)
                positive_df = pd.DataFrame(positive_data, columns=['seriesuid', 'center_i', 'center_r', 'center_c', 'category'])
            ...
            if negative is not None:
                ...
                negative_data = np.concatenate([pid_array, negative, category_array], axis=1)
                negative_df = pd.DataFrame(negative_data, columns=['seriesuid', 'center_i', 'center_r', 'center_c', 'category'])
        ...
        total_positive.to_csv(os.path.join(save_path, f'positive_IRC_{filename_key}.csv'))
        total_negative.to_csv(os.path.join(save_path, f'negative_IRC_{filename_key}.csv'))

    @staticmethod
    def get_luna16_crop_range(volume, nodule_center_xyz, crop_range, origin_xyz, spacing_xyz, direction_xyz):
        depth, height, width = volume.shape
        distance_threshold = np.linalg.norm([np.min(crop_range)//2, np.max(crop_range)//2])
        ...
        for candidate_center_index in range(crop_range[0]//2, depth-crop_range[0]//2, crop_range[0]):
            for candidate_center_row in range(crop_range[1]//2, height-crop_range[1]//2, crop_range[1]):
                for candidate_center_column in range(crop_range[2]//2, width-crop_range[2]//2, crop_range[2]):
                    candidate_center = np.array([candidate_center_index, candidate_center_row, candidate_center_column])

                    if nodule_center_xyz.shape[0] > 0:
                        for voxel_nodule_center in voxel_nodule_center_list:
                            center_distance = np.linalg.norm(candidate_center-voxel_nodule_center)
        ...
        # print(volume.shape, crop_range, len(positive_sample), len(negative_samples))
