...
class MatchingNetwork_3d(MatchingNetwork):
    def __init__(self, keep_prob, batch_size=32, num_channels=1, learning_rate=1e-3, fce=False, num_classes_per_set=20, \
                 num_samples_per_class=1, image_size=28, use_cuda=True, model_depth=50, n_classes=2):
        super(MatchingNetwork_3d, self).__init__(keep_prob, batch_size=32, num_channels=1, learning_rate=1e-3, fce=False, num_classes_per_set=20, \
                 num_samples_per_class=1, image_size=28, use_cuda=True)
        self.g = build_3d_resnet(
            model_depth=model_depth, n_classes=n_classes, 
            conv1_t_size=7, conv1_t_stride=2, keep_last_layer=False)
    ...
    def forward(self, support_set_images, support_set_y_one_hot, target_image, target_y):
        ...
        # calculate the accuracy
        values, indices = preds.max(1)
        target_y = torch.squeeze(target_y, dim=1)
        accuracy = torch.mean((indices.squeeze() == target_y).float())
        crossentropy_loss = F.cross_entropy(preds, target_y.long())

        return accuracy, crossentropy_loss, preds, indices.squeeze()