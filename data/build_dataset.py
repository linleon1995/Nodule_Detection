...
def random_rotate_3d(x, y):
    max_angle = 15
    for axis in [(0, 1), (0, 2), (1, 2)]:
    # for axis in range(1,2):
        if np.random.rand() > 0.5:
            angle = random.uniform(-max_angle, max_angle)
            x = scipy.ndimage.interpolation.rotate(x, angle, mode='nearest', axes=axis, reshape=False)
            y = scipy.ndimage.interpolation.rotate(y, angle, mode='nearest', axes=axis, reshape=False)
    return x, y
