...
def main(config_reference):
    ...
    config = configuration.get_device(config)
    ...
    model = build_3d_resnet(model_depth=50, n_classes=config.MODEL.NUM_CLASSES-1)
    ...
    valid_dataloader = DataLoader(valid_dataset, batch_size=config.DATA.BATCH_SIZE, shuffle=True, pin_memory=True, num_workers=0)

    # Logger
    LOGGER.info("Start Training!!")
    LOGGER.info("Training epoch: {} Batch size: {} Shuffling Data: {} Training Samples: {}".
            format(config.TRAIN.EPOCH, config.DATA.DATA_PATH, config.DATA.SHUFFLE, len(train_dataloader.dataset)))
